...
- 本地豆子、免费补给、分模式战绩、schema v2 JSON 档案与四种玩法确定性对局恢复
- 可从大厅进入、对两名 AI 完成叫抢与出牌、结算并再来一局的 Godot 斗地主试玩闭环，使用居中的正常比例重叠手牌和真实中央出牌节点
...
- 三种麻将共用的应用/表现契约、合法操作选项、独立会话适配器与种子+命令重放恢复
- xUnit 自动测试
...
直接试玩可运行 `./scripts/godot.sh --path .`。大厅中的斗地主、大众麻将、四川血战和四人日麻均可完整玩到结算；点击真实手牌后使用“出牌”或“换三张”，鸣牌、定缺、立直弃牌和其他选项会直接按规则引擎的合法操作列出，也可以使用“提示”或“托管”。四种玩法的未完成牌局都会自动保存，下次从大厅对应入口继续；斗地主也可在大厅明确放弃旧续局并开始新牌局。
...
│   │   ├── Mahjong/               # 麻将公共表现数据、命令恢复与三种独立会话适配器
