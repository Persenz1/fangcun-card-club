...
public sealed class StandardMahjongGameSession : IMahjongGameSession
{
    ...
    public static StandardMahjongGameSession Restore(MahjongRecoveryState recovery)
    {
        ArgumentNullException.ThrowIfNull(recovery);
        recovery.Validate();
        if (recovery.Mode != MahjongMode.Standard)
        {
            throw new InvalidDataException("恢复记录不是大众麻将。");
        }

        var session = new StandardMahjongGameSession(
            recovery.Seed,
            recovery.HumanSeat,
            recovery.BaseScore);
    ...
    private void Replay(IEnumerable<MahjongCommandRecord> records)
    {
        foreach (var record in records)
        {
            ...
            if (!result.Accepted)
            {
                throw new InvalidDataException($"大众麻将恢复命令无法重放：{result.Error}");
            }
